...
def _draw_matplotlib_plot(plot_df, **plot_params):
    try:
        ...
        raise ImportError(
            "matplotlib is required for using it as plotting backend. Please install it first."
        ) from e
    ...
    # Each chart has size plot_params["chart_width"] x plot_params["chart_height"] in px hence we need atleast
    # 3 times the height to fit all the charts
    # Setup base plot
    ...
    axes[0].plot(predicted_series["time"], predicted_series["value"], label="Mean")
    axes[0].plot(original_series["time"], original_series["value"], label="Observed")
    axes[0].fill_between(
    ...
    axes[1].plot(pointwise_series["time"], pointwise_series["value"], label="Pointwise")
    axes[1].fill_between(
...
def plot(ci_model, **kwargs) -> alt.Chart:
    ...
    Args:
      ...
      **kwargs: arguments for modifying plot defaults. static_plot - whether to
        ...
        interactive plot. backend - literal["altair","matplotlib","plotly"] to use
        for generating the figure alpha - float for determining confidence level for
    ...
    if plot_params["backend"] == "altair":
        ...
        raise ValueError(
            f"backend must be one of 'altair', 'matplotlib', or 'plotly'. Got {plot_params['backend']}."
        )
    return plt
...
def _create_plot_df(series: pd.DataFrame, alpha: float = 0.05) -> pd.DataFrame:
    ...
    plot_df["stat_pretty"] = pd.Categorical(
        plot_df["stat_pretty"], categories=["Observed", "Mean", "Median"], ordered=True
    )
...
def _create_plot_component_df(
    ...
    if all([x not in component for x in ["lines", "bands", "std"]]):
        raise ValueError(
            "`component` must be one of 'lines', 'bands', or 'std'."
            "Got %s." % component
        )
    ...
    sub_df["scale"] = sub_df["scale_stat"].str.replace(stats_to_drop, "", regex=True)
    sub_df.loc[sub_df["scale"].str.contains("observed|posterior"), "scale"] = "original"
    ...
    if component == "std":
        z_val = (
            tfp.distributions.Normal(loc=0.0, scale=1.0)
...
def _create_base_layers(plot_df: pd.DataFrame, **kwargs):
    ...
    # Only draw a line at the end of the pre-period if there are points between
    # it and the start of the post-period.
    if any((plot_df["time"] > pre_period_end) & (plot_df["time"] < post_period_start)):
...
def _draw_classic_plot(plot_df: pd.DataFrame, **kwargs) -> alt.Chart:
    ...
    chart_layers = tuple([layers["lines"], layers["band"], layers["hline"]] + vlines)
    final_plot = (
...
def _draw_interactive_plot(plot_df: pd.DataFrame, **kwargs) -> alt.Chart:
    ...
    selection_color = alt.condition(
        stat_selection, alt.Color("stat_pretty:N", legend=None), alt.value("lightgray")
    )
    legend = (
        alt.Chart(plot_df)
        .mark_point()
    ...
    static_layers["lines"] = static_layers["lines"].encode(color=static_color_spec)
